...
Nechť $n \geq n_0$ je pevné. Víme, že $f_n(x) \in R([a,b])$, tedy $\exists D$, že $S(f_n,D) - s(f_n,D)<\epsilon$
Nyní $$S(f,D) - s(f,D) \leq S(f_n+\epsilon, D) - s(f_n-\epsilon,D) \leq S(f_n,D)+S(\epsilon,D) - s(f_n,D) - s(\epsilon,D) \leq S(f_n,D) - s(f_n,D) +\epsilon(b-a) - (-\epsilon)(b-a) < \epsilon +2\epsilon(b-a)$$
\end{proof}

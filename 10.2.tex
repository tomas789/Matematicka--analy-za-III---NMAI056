...
Tedy 
$$\forall \varepsilon > 0 \ \exists n_0 \ \forall m,n \geq n_0 \textrm{ : } | \sum_{k = n}^m a_k | < \varepsilon$$
...
$$ | s_n(x) - s_m(x) | = | \sum_{k=n+1}^m u_k(x) | \leq \sum_{k=n+1}^m |u_k(x)| \leq \sum_{k=n+1}^m a_k < \textrm{BC} < \varepsilon $$
Tedy $$\sum u_n \con $$
\end{proof}
